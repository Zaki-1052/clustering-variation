...
│   └── results/     (10 files)  # barplot, pcoa, pca, amova, het, ibd, combined, elbow, delta_k, evanno
│
├── snp_analysis/                # SNP ADMIXTURE analysis
│   ├── scripts/     (15 files)  # snpk/ Python package + SLURM scripts
│   ├── inputs/       (8 files)  # hgdp_pruned.*, hgdp_qc.*, psam, pca, ldprune
│   ├── output/      (50 files)  # K*_rep*.log from admixture runs
│   ├── results/      (9 files)  # barplot, cv, pca, amova, het, ibd, combined, snp_barplot_K7
